...
![Correlation Matrix of Numeric Predictors with G3](graphs/correlation_matrix.png){#fig-corr-matrix width=75%}

![Correlations Among Behavioral Predictors](graphs/behavioral_correlations.png){#fig-behavioral-corr width=75%}
...
![PCA Loadings for First Five Components](graphs/pca_loadings.png){#fig-pca-loadings width=70%}
...
![PCA Biplot: PC1 vs PC2 Colored by Final Grade](graphs/pca_biplot.png){#fig-pca-biplot width=75%}
...
![LASSO Coefficient Paths](graphs/lasso_coefficient_paths.png){#fig-lasso-path width=75%}
...
The word clouds are dominated by repeated categorical levels, such as `guardian_mother` and standard parental job labels. However, stratifying by performance reveals subtle shifts. The lower-performing group ($G3 < 10$) shows a higher prevalence of disengagement tokens (notably `activities_no` and `schoolsup_no`), while the higher-performing group ($G3 \ge 14$) shows comparatively more extracurricular participation (`activities_yes`). This suggests that while broader family contexts are consistent, engagement-related variables differentiate high performance more effectively than demographics alone.

### 6.5.2 Classification Performance (Neural Network vs Tree)
We compared the Multi-Layer Perceptron (MLP) against a standard Decision Tree to test if non-linear complexity improves prediction for high-performing students. On the held-out test set, both the Neural Network and the Decision Tree achieved an accuracy of **0.7000**. This matches the baseline accuracy exactly, indicating that neither model successfully distinguished top-tier students better than simply predicting "Not High Performing" for everyone. This reinforces the difficulty of predicting exceptional academic success using only demographic and background data without access to prior grade history.

### 6.5.3 Training Dynamics
![Neural Network Training vs Validation Loss](graphs/nn_loss_curve.png){#fig-loss-curve width=75%}
...
Connecting back to alcohol use, our feature importance analysis indicates that alcohol consumption is a lower-priority predictor compared to academic history (failures) and engagement (absences). It does not appear in the top 10 most important features, suggesting it is a weak standalone signal for high academic performance. In practice, this means that screening students based solely on alcohol use would likely generate many false positives. A more effective approach would be to treat alcohol as one piece of a broader profile, alongside student engagement and support indicators that stand out in the stratified word clouds.
