---
title: "Alcohol Consumption and Student Achievement: A Multi-Model Regression Comparison of Predictors of Final Grades"
author: 
  - "Anushka Basu"
  - "Sherelle Li"
  - "Yifeng Jin"
...
::: {layout-ncol=2}
![Distributions of Key Variables with Potential Outliers](graphs/distributions.png){#fig-distributions width=100%}

![Boxplots to Identify Outliers](graphs/boxplots_outliers.png){#fig-boxplots width=100%}
:::
...
![Distributions of Key Variables](graphs/eda_distributions.png){#fig-eda-dist width=50%}
...
::: {layout-ncol=2}
![Correlation Matrix of Numeric Predictors with G3](graphs/correlation_matrix.png){#fig-corr-matrix width=40%}

![Correlations Among Behavioral Predictors](graphs/behavioral_correlations.png){#fig-behavioral-corr width=40%}
:::
...
![Scree Plot and Cumulative Variance Explained by PCA](graphs/pca_variance.png){#fig-pca-variance width=60%}
...
::: {layout-ncol=2}
![PCA Loadings for First Five Components](graphs/pca_loadings.png){#fig-pca-loadings width=40%}

![PCA Biplot: PC1 vs PC2 Colored by Final Grade](graphs/pca_biplot.png){#fig-pca-biplot width=40%}
:::
...
Both procedures were implemented on the training set only to avoid information leakage. All predictors were standardized prior to selection, and categorical variables were included through one-hot encoding. Two stepwise models were considered: An AIC-based model, which prioritizes predictive accuracy and allows a larger number of predictors. A BIC-based model, which applies a stronger penalty for model complexity and yields a more parsimonious specification.
...
**Baseline Linear Regression.** The baseline OLS model achieves an R² of 0.387 and adjusted R² of 0.338 with 40 predictors (F-test p < 0.001). None of the alcohol consumption measures are statistically significant once controls are included: `Dalc` (β = -0.208, p = 0.302), `Walc` (β = +0.040, p = 0.808), and `Talc` (β = -0.073, p = 0.272). This lack of significance reflects multicollinearity among alcohol variables and suggests alcohol consumption does not have a strong independent association with final grades. Academic history and aspirations dominate the results—past failures are the strongest negative predictor, while study effort and plans for higher education are positively associated with achievement (see Appendix Table A4 for significant predictors). These findings motivate stepwise selection and penalized regression to address multicollinearity.

**Stepwise Regression.** The AIC-based model retained 12 predictors, while the BIC-based model retained 7 (see Appendix Table A5). Both models retained failures, school_MS, higher_yes, Talc, schoolsup_yes, studytime, and health. Notably, total alcohol consumption (Talc) was retained in both models, whereas `Dalc` and `Walc` were excluded individually. Model fit comparison shows the AIC model (Adjusted R² ≈ 0.345) slightly outperformed the full baseline (Adjusted R² ≈ 0.338) despite using far fewer predictors, while the BIC model (Adjusted R² ≈ 0.326) achieved substantial parsimony with modest reduction in fit. In the AIC model, Talc shows a statistically significant negative coefficient—contrasting with the full baseline where alcohol variables were not significant due to multicollinearity. This supports the conclusion that alcohol consumption contributes modestly but non-negligibly to predicting academic performance when the model is restricted to informative predictors.

**Penalized Regression.** Figure @fig-lasso-path shows the LASSO coefficient paths as a function of the regularization parameter α (log scale). As α increases, coefficients shrink toward zero, with the dashed red line indicating the cross-validated optimal value (α ≈ 0.098).
...
![Model performance and complexity comparison](graphs/penalized_regression_comparison.png){#fig-model-performance width=40%}
...
::: {layout-ncol=3}
![Top 20 Important Features](graphs/RF_lettergrade_importantfeatures.png){#fig-RF_L_IF width=100%}

![Confusion Matrix](graphs/RF_lettergrade_confusionmatrix.png){#fig-RF_L_CM width=100%}

![ROC Curve](graphs/RF_lettergrade_ROC.png){#fig-RF_L_ROC width=100%}
:::
...
::: {layout-ncol=3}
![Top 20 Important Features](graphs/RF_performance_importantfeatures.png){#fig-RF_P_IF width=100%}

![Confusion Matrix](graphs/RF_performance_confusionmatrix.png){#fig-RF_P_CM width=100%}

![ROC Curve](graphs/RF_performance_ROC.png){#fig-RF_P_ROC width=100%}
:::
...
::: {layout-ncol=3}
![Top 20 Important Features](graphs/xg_lettergrade_importantfeatures.png){#fig-XG_L_IF width=100%}

![Confusion Matrix](graphs/xg_lettergrade_confusionmatrix.png){#fig-XG_L_CM width=100%}

![ROC curve](graphs/xg_lettergrade_ROC.png){#fig-XG_L_ROC width=100%}
:::
...
::: {layout-ncol=3}
![Top 20 Important Features](graphs/xg_performance_importantfeatures.png){#fig-XG_P_IF width=100%}

![Confusion Matrix](graphs/xg_performance_confusionmatrix.png){#fig-XG_P_CM width=100%}

![ROC curve](graphs/xg_performance_ROC.png){#fig-XG_P_ROC width=100%}
:::
...
## Neural Networks & Text Mining Results

### Qualitative Insights from Text Mining
![Word Clouds of Student Backgrounds by Performance Level](graphs/word_cloud_good_vs_bad.png){#fig-wordclouds width=50%}
...
### Classification Performance (Neural Network vs Tree)
...
### Feature Importance (Bagged Tree)
...
# Conclusion
## Linear, Stepwise, and Penalized Regression
Across all regression approaches, three consistent findings emerge. First, academic history and behavior—particularly past failures, study time, and educational aspirations—are the dominant predictors of final grade. These variables are statistically strong, stable across model specifications, and central to out-of-sample predictive accuracy. Second, model comparison reinforces that no single regression model drastically outperforms the rest in predictive performance; test RMSE values fall within a narrow range (~2.78–2.88), with Elastic Net performing best by a small margin. Penalized regression improves robustness and generalization relative to the full OLS baseline, while stepwise models improve interpretability by removing redundant predictors.
...
# Appendix
## Summary Statistics Tables and Selected Predictors Tables 
...
## Explanation of XGBoost Algorithm
